...
<?php
   ...
   *//*
   ...
   /*attempt 2
   $responses = array();
   $query_response = "select * from mismatch_response where user_id=" . $_SESSION['user_id'];
   $data_response = mysqli_query($dbc,$query_response)
        or die('trouble querying5');
   while($row = mysqli_fetch_array($data_response))
   {
        $query_category = "select name, category from mismatch_topic where topic_id=" . $row['topic_id'];
        $data_category = mysqli_query($dbc,$query_category)
            or die('something wrong');
        $row_category = mysqli_fetch_array($data_category);
        $row['category_name'] = $row_category['category'];
        $row['topic_name'] = $row_category['name'];
        
        array_push($responses, $row);
   }
   */ 
   $responses = array();
   $query_response = "select mr.response_id, mr.topic_id, mr.response, mt.name as topic_name, mc.name as category_name "
   ...
   <body>
        ...
        <?php 
            ...
            foreach($responses as $response)
            {
                ...
                if($current_legend != $response['category_name'])
                {
                    if($response['topic_id'] != 1)
                        echo '</fieldset>';
                    
                    echo '<fieldset><legend>' . $response['category_name'] . '</legend>';
                    $current_legend = $response['category_name']; 
                }

                echo '<label for="' . $response['topic_id'] . '">' . $response['topic_name'] . ': </label>';
                echo '<input type="radio" id="' . $response['topic_id'] . '" name="' . $response['topic_id'] . '" value="1" ';
                    if(isset($response['response']) && $response['response'] == 1)
                        echo 'checked="checked"';
                echo '>' . 'love  ';
                echo '<input type="radio" id="' . $response['topic_id'] . '" name="' . $response['topic_id'] . '" value="2" ';
                    if(isset($response['response']) && $response['response'] == 2)
                        echo 'checked="checked"';
                echo '>' . 'hate';
                echo '<br>';
            }
